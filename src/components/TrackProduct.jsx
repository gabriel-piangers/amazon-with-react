...
function TrackProduct({product}) {
    const progress = (dayjs()-dayjs(product.placedTime))/(dayjs(product.arrivingOn).subtract(dayjs()))
    ...
    return (
        <div className="py-6 flex flex-col gap-10 w-[90%] max-w-[1000px] mx-auto">
            ...
            <div>
                <div className="text-2xl font-medium my-2">Arriving on {dayjs(product.arrivingOn).format("dddd, MMMM, D")}</div>
                <div>{product.name}</div>
                <div className="mb-8">Quantity: {product.quantity}</div>
                <div>
                    <img src={`../../${product.image}`} className="h-[200px] w-[200px] object-contain mb-4"/>
                </div>
            </div>
            ...
            <div className="flex justify-between p-2 text-xl">
                ...
                <p className={`${progress<1 && progress>0.5 && 'text-green-700'}`}>Shipped</p>
            ...
            <div className="border border-stone-400 shadow-md  h-[30px] rounded-2xl overflow-hidden">
                <div className={`bg-green-700 w-[10%] h-[100%] rounded-2xl`} style={{
                    width: `${progress * 100}%`
                }}></div>
            </div>
            </div>
