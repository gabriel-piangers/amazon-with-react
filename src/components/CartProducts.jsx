...
function CartProducts({ cart, dispatchCart, dispatchCartQuantity }) {
  ...
  function setDeliveryOption(deliveryId, productId) {
    ...
    if (selected) {
      dispatchCart({
        type: "update",
        payload: { id: productId, update: { deliveryOption: selected } },
      });
    }
  }
  ...
  if (cart.length > 0) {
                  ...
                  {deliveryOptions.map((option) => (
                    <div className="flex" key={option.id}>
                      <input
                        ...
                        checked={
                          cartItem.deliveryOption.id === option.id
                            ? true
                            : false
                        }
                        onChange={() => {}}
    ...
    return (
      <div className="w-full max-w-[900px] p-4 my-6 max-lg:order-2">
        ...
        <div className="my-4">
          <NavLink to={"/"} className="px-6 py-1 bg-yellow-400 rounded-md">
            View Products
          </NavLink>
        </div>
      </div>
